...
# -*- coding: utf-8 -*-
"""
Multi-Task Drug Sensitivity Classification Pipeline (GDSC)

Parallelised for many-core machines.

Inputs  (produced by transform_for_multitask_zscore.py):
  {output_prefix}_binary_matrix.csv   — Drug Name × cell-line binary labels (0/1/NaN)
  {output_prefix}_zscore_matrix.csv   — raw Z scores (used when --z_threshold is set)
  {output_prefix}_smiles_mapping.csv  — Drug_Name, SMILES  (fallback)

Binary label: 1 = sensitive (Z ≤ threshold), 0 = resistant, NaN = untested.

Fingerprints : Avalon, ECFP, ECFP_Count, MACCS
Classifiers  : RandomForest, SVM, ExtraTrees, LogisticRegression,
               KNeighbors, GradientBoosting, GaussianNB
Splits       : random, scaffold, butina, umap_cluster

Parallelism
-----------
* --max_workers N  : outer-loop workers (default = all CPU cores).
                     With 32 cores and 112 combinations the full run
                     completes in roughly the time of ~4 sequential combos.
* --n_jobs_clf N   : n_jobs inside each classifier (default = 1 when
                     max_workers > 1 to avoid over-subscription;
                     auto-set to -1 when max_workers = 1).
* Fingerprint generation is also parallelised across compounds with joblib.

Model saving (one pickle per FP × Classifier × Split):
  {'models': [clf_per_cell_line, ...], 'scaler': scaler,
   'thresholds': [...], 'cell_lines': [...], ...}
"""
...
def _worker_combination(args):
    ...
    (data_folder, output_prefix, z_threshold,
     ...
     results_path, models_path, run_cv, n_jobs_clf) = args
...
class MultiTaskDrugClassifier:
    ...
    def __init__(self, results_path="./results_gdsc", models_path="./models_gdsc",
                 n_jobs=1):
        ...
        self.binary_data = None   # Drug × cell-line binary labels (DataFrame)
        self.smiles_data = {}     # drug_name → SMILES
        self.fingerprints = {}    # fp_type → DataFrame

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.results_file = self.results_path / f"gdsc_multitask_results_{timestamp}.csv"
    ...
    def load_data(self, data_folder, output_prefix, z_threshold=None, verbose=True):
        """
        Load binary labels and SMILES from the transform output folder.
        z_threshold: if set, recompute binary labels from the Z-score matrix
                     instead of reading the pre-saved binary_matrix.csv.
        verbose: set False in workers to suppress output.
        """
        ...
        if z_threshold is not None:
            ...
            if not binary_file.exists():
                raise FileNotFoundError(
                    f"Binary matrix not found: {binary_file}\n"
                    "Run transform_for_multitask_zscore.py first, or pass "
                    "z_threshold= to recompute from the Z-score matrix."
                )
    ...
    def generate_fingerprints(self, fp_types=None, n_jobs=-1):
        """Generate fingerprint DataFrames for all requested types (n_jobs=-1 = all cores)."""
        if fp_types is None:
    ...
    def _proba_positive(clf, X):
        """P(positive class); returns zeros when only one class was seen in training."""
    ...
    def _train_per_cell_line(self, clf_proto, X_train, X_test, y_train_df, y_test_df):
        """Train one fresh classifier clone per cell line; return model list + metadata."""
        ...
        for cell_line in y_train_df.columns:
            ...
            # Fresh clone — no state leak between cell lines
            clf = clf_proto.__class__(**clf_proto.get_params())
            clf.fit(X_tr_sc[tr_mask], y_tr)
            individual_models.append(clf)

            if len(y_te) > 0 and len(y_te.unique()) >= 2:
                thr = self._youden_threshold(y_te, self._proba_positive(clf, X_te_sc[te_mask]))
            else:
                thr = 0.5
    ...
    def train_and_evaluate(self, fp_type, clf_name, split_method, run_cv=True):
        """Train and evaluate one (fp_type, clf_name, split_method) combination."""
        ...
        individual_models, scaler, thresholds, per_cell = \
            self._train_per_cell_line(clf_proto, X_train, X_test, y_train, y_test)
        ...
        if run_cv:
            ...
            for fold, (tr_idx, te_idx) in enumerate(
                ...
                _, scaler_cv, _, per_cell_cv = self._train_per_cell_line(
                    clf_proto,
    ...
    def run_pipeline(self, data_folder, output_prefix,
        ...
        """Run the full FP × classifier × split pipeline. max_workers=-1 = os.cpu_count()."""
        t0 = time.time()
        ...
        print(f"\n{'='*60}")
        print(f"GDSC Multi-Task Classification Pipeline")
        print(f"  CPU cores       : {n_cpu}")
        print(f"  Max workers     : {max_workers if parallel else 1} (sequential)")
        print(f"  n_jobs_clf      : {n_jobs_clf}")
        print(f"  FP gen jobs     : {fp_gen_jobs}")
        print(f"  Combinations    : {total}  "
              f"({len(fp_types)} FP × {len(clf_names)} clf × {len(split_methods)} splits)")
        print(f"  CV              : {'yes (5-fold)' if run_cv else 'no'}")
        print(f"  Results file    : {self.results_file}")
        print(f"  Models folder   : {self.models_path}")
        print(f"{'='*60}\n")
        ...
        if parallel:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as pool:
                ...
                for future in pbar:
                    ...
                    if err:
                        tqdm.write(f"  ERROR [{fp_}|{clf_}|{split_}]: {err[:120]}")
                    else:
            ...
            self.classifiers = {
                name: clf.__class__(**{**clf.get_params(), 'n_jobs': n_jobs_clf}
                                   if hasattr(clf, 'n_jobs') else clf.get_params())
                for name, clf in self.classifiers.items()
            }
            self.classifiers = {
                'RandomForest':       RandomForestClassifier(random_state=42, n_jobs=n_jobs_clf),
                'SVM':                SVC(probability=True, random_state=42, max_iter=10000),
                'ExtraTrees':         ExtraTreesClassifier(random_state=42, n_jobs=n_jobs_clf),
                'LogisticRegression': LogisticRegression(random_state=42, max_iter=1000,
                                                         n_jobs=n_jobs_clf),
                'KNeighbors':         KNeighborsClassifier(n_jobs=n_jobs_clf),
                'GradientBoosting':   GradientBoostingClassifier(random_state=42),
                'GaussianNB':         GaussianNB(),
            }
            done = 0
        ...
        if self.results_file.exists():
            ...
            tt = df[df['Split'] == 'Train_Test']
            if not tt.empty:
                summary = (tt.groupby(['Split_Method', 'Fingerprint', 'Model'])
                           [['Accuracy', 'F1', 'AUC']].mean().round(3))
                print("\nSummary (Train_Test, mean across cell lines):")
                print(summary.to_string())


def main():
    parser = argparse.ArgumentParser(
        description="Multi-task GDSC drug sensitivity classifier (parallel)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument('--data_folder',   required=True,
                        help="Transform output folder (e.g. gdsc_nsclc_20May_output)")
    parser.add_argument('--output_prefix', required=True,
                        help="Prefix used by transform script (e.g. gdsc_nsclc_20May)")
    parser.add_argument('--results_path',  default='./results_gdsc')
    parser.add_argument('--models_path',   default='./models_gdsc')
    ...
    parser.add_argument('--z_threshold',   type=float, default=None,
                        help="Recompute binary labels from Z-score matrix at this "
                             "threshold (e.g. -1.0 → ~8%% sensitive). "
                             "Omit to use precomputed binary_matrix.csv.")
    parser.add_argument('--max_workers',   type=int, default=-1,
                        help="Parallel outer-loop workers. "
                             "-1 = all CPU cores (default), "
                             "1 = sequential, N = exactly N workers.")
    parser.add_argument('--n_jobs_clf',    type=int, default=None,
                        help="n_jobs inside each classifier. "
                             "Auto: 1 when parallel, -1 when sequential.")
    parser.add_argument('--fp_gen_jobs',   type=int, default=-1,
                        help="n_jobs for fingerprint generation (-1 = all cores).")
