...
# -*- coding: utf-8 -*-
"""
Multi-Task Drug Sensitivity Classification Pipeline (GDSC) — Unweighted

Baseline version of multitask_classifier_zscore.py with no class-imbalance
correction: all classifiers are trained with plain clf.fit(X, y) and Youden's J
post-hoc threshold on the test fold.

Uses deduplicated training files (smiles_mapping.csv / zscore_matrix.csv)
from the cleaned pipeline output folder.
"""
...
class MultiTaskDrugClassifier:
    ...
    def __init__(self, results_path="./results_gdsc_unweighted",
                 models_path="./models_gdsc_unweighted",
                 n_jobs=1,
                 save_models=True):
        ...
        self.results_file = (self.results_path /
                             f"gdsc_unweighted_results_{timestamp}.csv")
        self._results_header_written = False

        self.n_jobs      = n_jobs
        self.classifiers = self._build_classifiers(n_jobs)

    def _build_classifiers(self, n_jobs):
        return {
            'RandomForest':
    ...
    def run_pipeline(self, data_folder, output_prefix,
        ...
        print(f"\n{'='*60}")
        print(f"GDSC Multi-Task Classification Pipeline (Unweighted)")
        ...
        print(f"  Imbalance strategy : none (unweighted baseline)")
        ...
        if parallel:
            ...
            combo_args = [
                (str(data_folder), output_prefix, z_threshold,
                 ...
                 run_cv, n_jobs_clf, self.save_models)
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-task GDSC classifier — unweighted baseline (no imbalance correction).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument('--data_folder',    required=True)
    parser.add_argument('--output_prefix',  required=True)
    parser.add_argument('--results_path',   default='./results_gdsc_unweighted')
    parser.add_argument('--models_path',    default='./models_gdsc_unweighted')
