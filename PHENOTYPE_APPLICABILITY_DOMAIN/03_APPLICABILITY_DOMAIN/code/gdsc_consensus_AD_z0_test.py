# Consensus AD (CLASS-LAG + CONS-STD-PROB) — z <= 0.0, sample_weight=balanced
# 12-model ensemble: 3 FP (AVALON, ECFP, MACCS) x 4 algo (RF, LR, SVM, XGB)
...
TEST_SIZE    = 0.2
RANDOM_STATE = 42
COVERAGES    = [x / 100 for x in list(range(1, 10)) + list(range(10, 101, 10))]
...
RESULTS_CSV  = ROOT / 'PHENOTYPES_CLASSIFICATION_CODES_DATA' / 'results' / 'z0_weighted' / 'gdsc_weighted_results_20260521_154749.csv'
...
for drug, smi in smiles_map.items():
    ...
    if mol:
        ...
        n_fail += 1
print(f"  {len(fp_bank['AVALON'])} valid fingerprints  (failed: {n_fail})")

OUT_DIR = Path(__file__).resolve().parents[1] / 'results'
OUT_DIR.mkdir(parents=True, exist_ok=True)

print(f"\nrunning consensus AD ({TAG}) on {len(cell_lines)} cell lines")
print(f"  {N_JOBS_OUTER} outer x {N_JOBS_INNER} inner jobs  |  output: {OUT_DIR}")
...
out_per  = str(OUT_DIR / f'gdsc_consensus_AD_{TAG}_per_cellline.csv')
out_comp = str(OUT_DIR / f'gdsc_consensus_AD_{TAG}_comparison.csv')
out_summ = str(OUT_DIR / f'gdsc_consensus_AD_{TAG}_summary.csv')
...
print(f"\nResults ({TAG}):")
print(f"  cell lines: {len(df)}  |  consensus AUC: {df['Consensus_AUC'].mean():.4f}")
print(f"  mean CONS-STD: {all_constd.mean():.4f}  |  <0.10: {100*(all_constd<0.10).mean():.1f}%  |  <0.20: {100*(all_constd<0.20).mean():.1f}%")
print(f"\n  {'coverage':>9}  {'CLASS-LAG':>11}  {'CONS-STD':>10}  {'CONS-STD-PROB':>14}")
for cov, cl_c, cs_c, csp_c in zip(COVERAGES, cl_cols, cs_cols, csp_cols):
    print(f"  {int(cov*100):>8}%  {df[cl_c].mean():>11.4f}  {df[cs_c].mean():>10.4f}  {df[csp_c].mean():>14.4f}")
print(f"\n  best CLASS-LAG: {best_cl:.4f}  |  best CONS-STD: {best_cs:.4f}  |  best CONS-STD-PROB: {best_csp:.4f} ({best_csp_col})")
print(f"  baseline (100%): {baseline_auc:.4f}  |  max gain: {best_csp - baseline_auc:+.4f}")
...
summary_rows = [
    ...
    {'Metric': 'sample_weight',           'Value': 'balanced'},
