"""
CLASS-LAG Applicability Domain — z<=-0.75 threshold (sample_weight run)
------------------------------------------------------------------------
Source data : gdsc_weighted_results_20260522_140030.csv  (z<=-0.75, sample_weight)
Z threshold : -0.75  →  sensitive if z-score <= -0.75  (~13% sensitive)
AD method   : CLASS-LAG = min(p, 1-p)   (Sushko et al. 2010)

Input training folder : gdsc_multitask_threshold_0.75_output
TAG                   : z075
Output files prefixed : gdsc_classLAG_AD_z075_*
...
Z_THRESHOLD  = -0.75        # sensitive if z-score <= -0.75
TAG          = 'z075'
TEST_SIZE    = 0.2
RANDOM_STATE = 42
N_ESTIMATORS = 100
...
RESULTS_CSV  = ROOT / 'PHENOTYPES_CLASSIFICATION_CODES_DATA' / 'results' / 'z0.75_weighted' / 'gdsc_weighted_results_20260522_140030.csv'

# Training-folder files (deduplicated, 429 drugs, 108 NSCLC cell lines)
TRAIN_FOLDER = ROOT / 'PHENOTYPES_CLASSIFICATION_CODES_DATA' / 'input' / 'z0.75_threshold'
SMILES_CSV   = TRAIN_FOLDER / 'gdsc_multitask_threshold_0.75_smiles_mapping.csv'
ZSCORE_CSV   = TRAIN_FOLDER / 'gdsc_multitask_threshold_0.75_zscore_matrix.csv'
