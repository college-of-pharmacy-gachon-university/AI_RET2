"""
CONS-STD-PROB Applicability Domain — z<=0.0 threshold (sample_weight run)
---------------------------------------------------------------------------
Source data : gdsc_weighted_results_20260521_154749.csv  (z<=0.0, sample_weight)
Z threshold : 0.0   →  sensitive if z-score > 0.0  (~38% sensitive)

Changes vs. original gdsc_consensus_AD.py:
  - Fixed Z_THRESHOLD = 0.0  (replaces per-cell Youden threshold from old CSV)
  - sample_weight='balanced' added to all clf.fit() calls (RF, LR, SVM, XGB)
  - Cell line list taken from z0 results CSV
  - Output files prefixed gdsc_consensus_AD_z0_*
...
def process_cell(cell, zscore_col, z_threshold, fp_bank, n_jobs_inner):
    ...
    # Sample weights (consistent with weighted pipeline)
...
print(f"  sample_weight    : balanced")
print()
