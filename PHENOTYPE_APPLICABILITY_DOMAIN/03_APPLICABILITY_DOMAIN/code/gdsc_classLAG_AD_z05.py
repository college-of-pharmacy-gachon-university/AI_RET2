"""
CLASS-LAG Applicability Domain — z<=-0.5 threshold (sample_weight run)
-----------------------------------------------------------------------
Source data : gdsc_weighted_results_20260522_172746.csv  (z<=-0.5, sample_weight)
Z threshold : -0.5  →  sensitive if z-score <= -0.5  (~22% sensitive)
AD method   : CLASS-LAG = min(p, 1-p)   (Sushko et al. 2010)

Input training folder : PHENOTYPES_CLASSIFICATION_CODES_DATA/input/z0.5_threshold
TAG                   : z05
Output files prefixed : gdsc_classLAG_AD_z05_*
...
Z_THRESHOLD  = -0.5         # sensitive if z-score <= -0.5
TAG          = 'z05'
TEST_SIZE    = 0.2
RANDOM_STATE = 42
N_ESTIMATORS = 100
...
RESULTS_CSV  = ROOT / 'PHENOTYPES_CLASSIFICATION_CODES_DATA' / 'results' / 'z0.5_weighted' / 'gdsc_weighted_results_20260522_172746.csv'

# Training-folder files (deduplicated, 429 drugs, 108 NSCLC cell lines)
TRAIN_FOLDER = ROOT / 'PHENOTYPES_CLASSIFICATION_CODES_DATA' / 'input' / 'z0.5_threshold'
SMILES_CSV   = TRAIN_FOLDER / 'gdsc_multitask_threshold_0.5_smiles_mapping.csv'
ZSCORE_CSV   = TRAIN_FOLDER / 'gdsc_multitask_threshold_0.5_zscore_matrix.csv'
...
for drug, smi in smiles_map.items():
    ...
    if mol:
        ...
        fp_dict[drug] = arr
print(f"  Valid fingerprints: {len(fp_dict)} / {len(smiles_map)} drugs")
